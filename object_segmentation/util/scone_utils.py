...
def setup_device(params, ddp_rank=None):
    ...
    if params.ddp:
        ...
        print("Setup device", str(idr_torch.rank), " for Jean Zay training...")
...
def get_cameras_on_sphere(params, device, pole_cameras=False, n_elev=None, n_azim=None):
    """
    ...
    candidate_dist = torch.Tensor([params.camera_dist for i in range(n_camera)]).to(device)
...
def compute_occupancy_probability(scone_occ, pc, X, view_harmonics, mask=None,
    ...
    for i in range(n_loop):
        ...
        if i == q:
            up_idx = q * p + r
        preds_i = scone_occ(pc, X[:, low_idx:up_idx], view_harmonics[:, low_idx:up_idx])
