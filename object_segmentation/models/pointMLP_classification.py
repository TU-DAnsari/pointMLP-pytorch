...
class PointMLPBackbone(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        super(PointMLPBackbone, self).__init__()
...
class PointMLPClassHead(nn.Module):
        def __init__(self, class_num=40, embed_dim=64, activation="relu", **kwargs):
            super(PointMLPClassHead, self).__init__()
...
class PointMLP(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
            ...
            self.backbone = PointMLPBackbone(points=points, class_num=class_num, embed_dim=embed_dim, groups=groups, res_expansion=res_expansion,
            ...
            self.head = PointMLPClassHead(class_num=class_num, embed_dim=self.backbone.out_channels, activation=activation)

    def forward(self, x):
        x, x_pooled = self.backbone(x)
        return self.head(x_pooled)


def pointMLPClassification(num_points=1024, num_classes=40, **kwargs) -> PointMLP:
...
def pointMLPClassificationElite(num_points=1024, num_classes=40, **kwargs) -> PointMLP:
